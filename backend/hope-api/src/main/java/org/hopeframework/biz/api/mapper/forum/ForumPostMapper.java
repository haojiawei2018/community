...
public interface ForumPostMapper extends BaseMapper<ForumPost> {
    ...
    @Select("SELECT COUNT(*) FROM forum_post WHERE author_member_id = #{memberId} AND status = 'PUBLISHED' AND deleted = 0")
    long countPublishedByAuthor(@Param("memberId") Long memberId);

    @Select("SELECT COALESCE(SUM(like_count), 0) FROM forum_post WHERE author_member_id = #{memberId} AND status = 'PUBLISHED' AND deleted = 0")
    long sumReceivedLikesByAuthor(@Param("memberId") Long memberId);
